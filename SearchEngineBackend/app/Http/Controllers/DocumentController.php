...
class DocumentController extends Controller
{
    ...
    public function deleteOldDocumentsByYears(Request $request)
    {
        $yearsOld = intval($request->query('years')); 

        if (!is_numeric($yearsOld) || $yearsOld <= 0) {
            return response()->json([
                'message' => 'years must be a positive number'
            ], 400);
        }

        // Cutoff = today minus X years
        $cutoffDate = now()->subYears($yearsOld)->toDateString();

        try {
            $totalDeleted = 0;

            $documentsToDelete = FileDocument::whereDate('indexed_at', '<', $cutoffDate)->get();
            ...
            return response()->json([
                'message' => "$totalDeleted documents older than $yearsOld year(s) have been deleted",
            ]);
        } catch (Exception $e) {
            \Log::error("Error during year-based document cleanup: " . $e->getMessage());
    ...
    {
        try {
            $client = new MeiliClient(env('MEILISEARCH_HOST'), env('MEILISEARCH_KEY'));
            $indexName = 'file_documents'; // change if your index name differs
            ...
            return response()->json([
                'message' => "Index '$indexName' has been fully reset (dropped and recreated)."
            ]);
        } catch (Exception $e) {
            \Log::error("Failed to reset Meilisearch index: " . $e->getMessage());

            return response()->json([
                'message' => 'An error occurred while resetting the index',
                'error'   => $e->getMessage()
            ], 500);
        }
    }
